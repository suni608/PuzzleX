...
import type { KillerCage } from "../utils/types";
...
export const KillerGrid: React.FC<KillerGridProps> = ({
  grid,
  initialGrid,
  cages,
  activeCell,
  ...
  const cellMap = useMemo(() => {
    const map: Record<string, { cageIdx: number; isTopLeft: boolean; target: number }> = {};
    
    cages.forEach((cage, idx) => {
      // Find top-left cell of cage to display target
    ...
    <div className="grid grid-cols-9 gap-0.5 bg-slate-800 p-2 rounded-2xl shadow-2xl border border-slate-700 max-w-[450px] mx-auto aspect-square overflow-hidden">
      {grid.map((row, r) =>
        row.map((val, c) => {
          ...
          const borderBottomDash = r < 8 && cellMap[`${r+1}-${c}`]?.cageIdx !== currentCageIdx ? "border-b border-b-dashed border-b-slate-500" : "";
          const borderLeftDash = c > 0 && cellMap[`${r}-${c-1}`]?.cageIdx !== currentCageIdx ? "border-l border-l-dashed border-l-slate-500" : "";
          const borderRightDash = c < 8 && cellMap[`${r}-${c+1}`]?.cageIdx !== currentCageIdx ? "border-r border-r-dashed border-r-slate-500" : "";

          // Outer box borders for Sudoku regions
          const borderRightSolid = (c === 2 || c === 5) ? "border-r-2 border-r-slate-600" : "";
          const borderBottomSolid = (r === 2 || r === 5) ? "border-b-2 border-b-slate-600" : "";

          // Combined cell background style
          ...
          return (
            <div
              ...
              className={`relative flex items-center justify-center aspect-square select-none cursor-pointer text-xl transition-all duration-150 rounded-md
                ...
                ${borderRightSolid} ${borderBottomSolid}
                hover:bg-slate-700/30 active:scale-95`}
            >
              {/* Cage Target Clue in top-left cell */}
              {cageInfo?.isTopLeft && (
